...
internal static class DtoGenerator
{
    ...
    )
    {
        foreach (var type in dtoTypes)
        {
            ...
            GenerateDto(type, couldImportTypes2, config);
        }
    }

    public static void GenerateDto(Type type, List<Type> couldImportTypes, DtoGenerationConfig config)
    {
        var dtoString = GenerateDtoToString(type, couldImportTypes, config);

        var filePath = Path.Combine(config.OutputPath, $"{GetExtensionlessTypeScriptFilename(type)}.ts");
    ...
    internal static string GenerateDtoToString(Type type, List<Type> couldImportTypes, DtoGenerationConfig config)
    {
        ...
        var typeName = GetTypeScriptTypeName(type);

        if (config.ExportTypeNameAsConstant(type))
        {
            interfaceBuilder.AppendLine($"export const {typeName}TypeName = '{typeName}'");
        }

        interfaceBuilder.AppendLine($"export interface {typeName}{extendsClause} {{");
